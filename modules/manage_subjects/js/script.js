...
$("#btn-update").bind(
	"click",
	function(){
		saveUp("qualification.update_subject");
	}
);
...
$("#btn-delete").bind(
	"click",
	function(){
		saveUp("qualification.delete_subjects");
	}
)
function saveUp(task){
	var Rows = $("#table-subjects").find("tr");
	...
	$.ajax({
		data: dat,
		 type:"post", url: "modules/manage_subjects/ajax_EditSubjects.php", 
		 success: stuffEdited, 
...
function stuffEdited(dat){
	...
	obj = evaluate(dat);
	var ids;
	if("qualification.delete_subjects"==obj.task){
			ids = obj.Ids.split(",");
