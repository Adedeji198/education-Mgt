$("#btn-update").bind(
	"click",
	function(){
		saveUp("qualification.update_exams");
	}
);
...
$("#btn-delete").bind(
	"click",
	function(){
		saveUp("qualification.delete_exams");
	}
)
function saveUp(task){

	var Rows = $("#table-exams").find("tr");
	...
	$.ajax({
		data: dat,
		 type:"post", url: "modules/manage_exams/ajax_manage_exams.php", 
		 success: stuffEdited, 
...
function stuffEdited(dat){
	...
	obj = evaluate(dat);
	var ids;
	if("qualification.delete_exams"==obj.task){
			ids = obj.Ids.split(",");
