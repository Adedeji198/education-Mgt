...
function saveUp(task){
	...
	if("admission.matric" != task){	
		dat = {"data": dat_,"task":task };
	}else{
		dat = {"data":"empty", "task": task};
	}

	$.ajax({
		data: dat,
		 type:"post", url: "modules/manage_admiission/ajax_EditAdm.php", 
		 success: stuffEdited, 
...
JsonObj.Url = "modules/manage_admiission/ajax_FetchAdm.php";
...
JsonObj.callBack = searchArrived
JsonObj.Url = "modules/manage_admiission/ajax_FetchAdm2.php";
...
function getCandidateResult() {
	var dat; 
	 ...
	 $.ajax({ 
		data: dat,
		 type:"post",
		 url: "modules/manage_admiission/ajax_GetResltDetails.php",
